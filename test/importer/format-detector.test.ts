...
describe('detectFormat', () => {
  ...
  it('detects fetch format', () => {
    expect(detectFormat("fetch('https://api.example.com/users')")).toBe('fetch');
  });

  it('detects await fetch format', () => {
    expect(detectFormat("await fetch('https://api.example.com/users')")).toBe('fetch');
  });
