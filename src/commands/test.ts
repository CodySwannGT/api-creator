...
export const testCommand = new Command('test')
  ...
  .option('--dir <path>', 'Directory containing the generated client', './generated')
    ...
    try {
      ...
      if (options.list) {
        ...
        for (const ep of endpoints) {
          const params = ep.params.length > 0 ? chalk.gray(`(${ep.params.join(', ')})`) : '';
          console.log(`  ${chalk.white(ep.method.padEnd(7))} ${chalk.cyan(ep.name)}${params}`);
        }
        console.log('');
        return;
      }
      ...
      if (!authConfig.hasAuth) {
        ...
        console.log(chalk.gray('  Use --cookie, --token, or --api-key to authenticate.\n'));
      }
      ...
      if (healthEndpoint) {
        await testEndpoint(baseUrl, healthEndpoint, authConfig.headers, 'health check');
      }
      ...
      console.log('');
      if (failed === 0) {
        console.log(chalk.green.bold(`  All ${passed} endpoint(s) responded successfully.`));
      } else {
...
function parseEndpoints(source: string): ParsedEndpoint[] {
  ...
  while ((match = methodRegex.exec(source)) !== null) {
    ...
    methodRegex.lastIndex = nextMethodMatch ? nextMethodMatch.index : source.length;
    ...
    const params = paramsStr.trim() ? paramsStr.split(',').map((p) => p.trim().split(':')[0].trim()).filter(Boolean) : [];
    const hasBody = params.includes('body');
    const hasQueryParams = params.some((p) => p === 'options?') || paramsStr.includes('options?');

    endpoints.push({ name, method: httpMethod, path, params, hasBody, hasQueryParams });
  }
...
function buildAuthHeaders(
  ...
  if (options.apiKey) {
    ...
    const apiKeyMatch = source.match(/headers\.set\('(X-[^']+)',\s*this\.auth\.apiKey\)/);
...
function findHealthCheckEndpoint(endpoints: ParsedEndpoint[]): ParsedEndpoint | null {
  // Find a simple GET with no params — good for a health check
  return endpoints.find((e) => e.method === 'GET' && e.params.length === 0 && !e.hasBody) ?? null;
}
...
async function testEndpoint(
  ...
  if (path.includes(':param') || path.includes(':id') || path.includes(':hash')) {
    console.log(
      chalk.gray(`  skip  `) +
  ...
  try {
    ...
    if (status === 401 || status === 403) {
      console.log(chalk.yellow('        ^ Session expired or invalid auth. Use --cookie or --token.'));
      return false;
    }
