...
export function emitProjectBinEntry(_name: string): string {
  const lines = ["#!/usr/bin/env node", "import '../dist/cli.js';", ""];
  return lines.join("\n");
}
...
export function emitCommandsModule(
  ...
  for (const endpoint of endpoints) {
    const methodName = pathToMethodName(
      endpoint.method,
    ...
    if (isGraphQL) {
      ...
      const bakedOperationName =
        operationNameQp.observedValues[0] ?? commandName;
      ...
      for (const pp of pathParams) {
        lines.push(
          `    .argument('<${pp.name}>', '${pp.name} path parameter')`
        );
      }
      ...
      for (const vf of varFields) {
        ...
        lines.push(
          `    .option('--${vf.kebabName} <value>', '${vf.camelName} variable (e.g. "${safeExample}")')`
        );
      }
      ...
      for (const vf of varFields) {
        ...
        lines.push(
          `          if (options[${JSON.stringify(optionAccessor)}] !== undefined) variables[${JSON.stringify(vf.camelName)}] = options[${JSON.stringify(optionAccessor)}];`
        );
      }
      lines.push("        }");
