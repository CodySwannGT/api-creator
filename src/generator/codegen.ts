...
export async function generateClient(options: GenerateOptions): Promise<void> {
  ...
  if (auth.length > 0) {
    console.log(
      chalk.gray(`  Detected auth: ${auth[0].type} (${auth[0].key})`)
    );
  }
  ...
  console.log(
    chalk.gray(
      `  Grouped into ${endpoints.length} endpoints (base: ${resolvedBaseUrl})`
    )
  );
  ...
  console.log(
    chalk.gray(
      `  Inferred ${types.length} response types, ${requestTypes.length} request types`
    )
  );
