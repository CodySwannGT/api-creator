...
const SKIP_HEADERS = new Set([
  'accept', 'accept-encoding', 'accept-language', 'cache-cache',
  'connection', 'content-length', 'content-type', 'host', 'origin',
  'referer', 'sec-ch-ua', 'sec-ch-ua-mobile', 'sec-ch-ua-platform',
  'sec-fetch-dest', 'sec-fetch-mode', 'sec-fetch-site', 'sec-fetch-user',
  'upgrade-insecure-requests', 'user-agent', 'pragma', 'dnt',
  'if-none-match', 'if-modified-since', 'sec-ch-ua-platform-version',
  'sec-ch-device-memory', 'sec-ch-dpr', 'sec-ch-viewport-width',
  'ect', 'priority', 'cache-control',
]);
...
export async function captureAuth(url: string): Promise<AuthConfig | null> {
  const userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'api-creator-auth-'));
  ...
  console.log(chalk.white.bold('  Once logged in, press Enter here to capture auth.\n'));
  ...
  console.log(chalk.gray('  Capturing auth... Click around in the browser to trigger API calls.\n'));
  ...
  const auth = await new Promise<AuthConfig | null>((resolve) => {
    ...
    context.on('request', (request: PwRequest) => {
      void (async () => {
        ...
        if (capturedCookie || capturedToken) {
          ...
          console.log(chalk.cyan(`  ✓ ${request.method()} ${shortPath} (${capturedCount}/${MIN_CAPTURE_REQUESTS})`));
    ...
    const timeout = setTimeout(() => {
      if (!resolved) {
        ...
        if (capturedCookie) auth.cookie = capturedCookie;
        if (capturedToken) auth.token = capturedToken;
        if (Object.keys(capturedExtraHeaders).length > 0) auth.extraHeaders = { ...capturedExtraHeaders };
