...
export function registerProjectCommands(program: Command): void {
  ...
  for (const projectName of projects) {
    ...
    const projectCmd = program
      .command(projectName)
      .description(`CLI for ${manifest.originalUrl} (${manifest.endpoints.length} endpoints)`);
...
function registerAuthCommands(
  ...
  authCmd
    ...
    .option('-f, --file <path>', 'Read auth from a cURL command file instead of browser')
    .option('--stdin', 'Read auth from piped stdin (e.g. pbpaste | api-creator ...)')
    .action(async (options: { file?: string; stdin?: boolean }) => {
      // File or explicit --stdin: parse cURL
      if (options.file || options.stdin || (!process.stdin.isTTY && options.file !== undefined)) {
        let input: string;
        if (options.file) {
      ...
      if (auth) {
        ...
        console.error(chalk.red('No auth detected. Make sure you logged in and browsed around.'));
        process.exit(1);
      }
    });
  ...
  authCmd
    ...
    .action(() => {
      ...
      if (!auth) {
        console.log(`Not configured. Run: api-creator ${projectName} auth setup`);
        return;
      }
      ...
      if (auth.extraHeaders)
        console.log(`  Extra headers: ${Object.keys(auth.extraHeaders).length}`);
    });
...
function registerEndpointCommand(
  ...
  if (endpoint.isGraphQL) {
    ...
    cmd.option('--variables <json>', 'Raw variables JSON (overrides individual variable options)');
  }

  // Query params as options
  for (const qp of endpoint.queryParams) {
    if (qp.defaultValue !== undefined) {
      cmd.option(`--${qp.name} <value>`, `${qp.name} query parameter`, qp.defaultValue);
    } else {
  ...
  if (endpoint.hasBody) {
    ...
    cmd.option('--json <json>', 'Request body as JSON string (alias for --body)');
  }
  ...
  cmd.action(async (...actionArgs: unknown[]) => {
    ...
    const pathArgValues: string[] = endpoint.pathParams.map((_, i) => actionArgs[i] as string);
    const options = actionArgs[endpoint.pathParams.length] as Record<string, string | boolean | undefined>;

    // Get auth
    const auth = loadAuth(projectName);
    if (!auth) {
      console.error(
        chalk.red(`No auth configured. Run: api-creator ${projectName} auth setup`),
      );
      process.exit(1);
    }
    ...
    try {
      ...
      if (error instanceof HttpError) {
        if (error.status === 401 || error.status === 403) {
          ...
          console.error(chalk.red(`HTTP ${error.status}: ${error.body || error.message}`));
        }
        process.exit(1);
      }
      console.error(
